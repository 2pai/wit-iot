...
- [Build Smart Assistant with Wit.ai to Manage IoT Device](#build-smart-assistant-with-witai-to-manage-iot-device)
  ...
  - [Building Application](#building-application)
    - [How will It Work?](#how-will-it-work)
      - [Here is the flow in action](#here-is-that-flow-in-action)
...
Before we start building the app, I will describe how the app work and prerequisite to follow. Then, I will describe the detail of all the steps before it is ready to be run in production mode.
...
The way this application works is quite simple. But in this tutorial, we're using so much technology to achieve the goals. This is used so that the application that we create this time can be easily scaled with broader goals.
...
To give a clear details about how this app work in the background
...
The first thing we have to do is preparing the environment for developing our application. We will use [typescript](https://www.typescriptlang.org/) and [npm](https://www.npmjs.com) to develop our app. Make sure Typescript has installed. If not, run command `npm install -g typescript` to install typescript.
...
In this tutorial, we will use [wit.ai](https://wit.ai/) as an NLP system. With this, we can build our personal assistant as natural as possible and we don;t have to do a lot of Machine Learning coding, like using PyTorch, Tensorflow, etc, to train our application.
...
In this file, we have a function `get(message)` that is used to query messages into our Wit App. This function takes the message as a query then it will return 4 fields, such as Text, Intent, Trait, an Entity. All of the fields have a `confidence` to shows the percentage accuracy of predictions.
...
This connector also includes a schema of the data and `Init()` function is used to initialize connection to MongoDB when the app is started.

### Create MQTT Connector

To communicate with the IoT devices via API Gateway, we will use the MQTT Protocol(TCP). The main reason why we use the MQTT Protocol is to make our application can handle requests in asynchronous mode and don't block the other processes. For other reasons, MQTT is the protocol that is commonly used to communicate with IoT devices.
...
As in the introduction section, the goal of our IoT device is watering the plants using a pump. So, we will make sure that our IoT Device can do 3 things:
...
We will have 4 external libraries. Please make sure that you have already installed those libraries in Arduino IDE.
...
Please make sure the wiring pin is correct as the schema below.
...
4. If there's a message coming from topic `pompa` or device name topic, then the device will execute the command to turn on/off the pump based on data payload. After the command is executed, a status will be sent to `pompa-report` as feedback encoded in JSON.
